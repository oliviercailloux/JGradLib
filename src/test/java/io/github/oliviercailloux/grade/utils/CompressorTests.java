...
public class CompressorTests {
  ...
  void testCompress() throws Exception {
    ...
    final IGrade obtained =
        Compressor.compress(deflatedGrade, GradeStructure.from(ImmutableSet.of("g1", "g2")));
  ...
  void testCompressStruct() throws Exception {
    ...
    final IGrade expected = WeightingGrade
        .from(ImmutableMap.of(CriteriaPath.from("c1"), WeightedGrade.given(Mark.one(), 1d),
            CriteriaPath.from("c2/sub"), WeightedGrade.given(Mark.one(), 1d)));
  ...
  void testCompressTwoTwo() throws Exception {
    final GradeStructure model =
        GradeStructure.from(ImmutableSet.of("user.name", "main/Class1", "main/Class2"));
    final WeightedGrade one = WeightedGrade.given(Mark.one(), 1d);
    final IGrade grade = WeightingGrade.from(ImmutableMap.of(CriteriaPath.from("user.name"), one,
        CriteriaPath.from("main/Impl/Class1"), one, CriteriaPath.from("main/Impl/Class2"), one,
        CriteriaPath.from("main/Warnings"), one));
    final IGrade obtained = Compressor.compress(grade, model);
