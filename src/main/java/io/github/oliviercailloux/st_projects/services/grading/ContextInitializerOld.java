...
public class ContextInitializerOld implements GitFullContext, GradingContexter {

	public static ContextInitializerOld with(Supplier<RepositoryCoordinates> coordinatesSupplier) {
		final String tmpDir = System.getProperty("java.io.tmpdir");
		return new ContextInitializerOld(coordinatesSupplier, Paths.get(tmpDir), Instant.MAX);
	}

	public static ContextInitializerOld withPathAndIgnore(Supplier<RepositoryCoordinates> coordinatesSupplier,
			Path projectsBaseDir, Instant ignoreAfter) {
		return new ContextInitializerOld(coordinatesSupplier, projectsBaseDir, ignoreAfter);
	}

	public static ContextInitializerOld withIgnore(Supplier<RepositoryCoordinates> coordinatesSupplier,
			Instant ignoreAfter) {
		final String tmpDir = System.getProperty("java.io.tmpdir");
		return new ContextInitializerOld(coordinatesSupplier, Paths.get(tmpDir), ignoreAfter);
	}
	...
	private ContextInitializerOld(Supplier<RepositoryCoordinates> coordinatesSupplier, Path projectsBaseDir,
