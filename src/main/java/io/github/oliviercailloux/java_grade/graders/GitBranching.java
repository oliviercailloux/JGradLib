...
public class GitBranching implements GitGrader {
	...
	public WeightingGrade grade(GitWork work) throws IOException {
		...
		final Function<GitPathRootShaCached, Set<GitPathRootShaCached>> graphSiblings = r -> getSingleParent(history, r)
				.map(p -> history.graph().successors(p)).orElse(ImmutableSet.of(r));
		final TFunction<GitPathRootShaCached, Set<GitPathRootShaCached>, IOException> siblings = r -> graphSiblings
				.apply(r);
		final TPredicate<GitPathRootShaCached, IOException> hasAFewSiblings = compose(siblings,
				s -> 2 <= s.size() && s.size() <= 3);
...
//		final TPredicate<GitPathRootSha, IOException> aBranch = compose(history::getRefsTo, anyMatch(isBranch("br1")));
		final TPredicate<GitPathRootShaCached, IOException> aBranch = compose(p -> GradeUtils.getRefsTo(refs, p),
				anyMatch(isRefBranch("br1")));
		{
			final Pattern helloPattern = Marks.extendWhite("hello\\h+world");
			final CriterionGraderWeight<GitPathRootShaCached> graderBr1 = CriterionGraderWeight
					.given(Criterion.given("Branch br1"), aBranch, 2d);
			final CriterionGraderWeight<GitPathRootShaCached> graderContent = CriterionGraderWeight.given(
		...
		final TFunction<GitPathRootShaCached, Set<GitPathRootShaCached>, IOException> fatherSiblings = r -> getSingleParent(
				history, r).map(graphSiblings).orElse(ImmutableSet.of());
		final TPredicate<GitPathRootShaCached, IOException> hasCTopo = compose(fatherSiblings,
				s -> 2 <= s.size() && s.size() <= 3);
		final TPredicate<GitPathRootShaCached, IOException> cBranch = compose(p -> GradeUtils.getRefsTo(refs, p),
				anyMatch(isRefBranch("br2")));
		{
			...
			final CriterionGraderWeight<GitPathRootShaCached> graderBr2 = CriterionGraderWeight
		...
		}
		{
			...
			final TFunction<GitPathRootShaCached, ImmutableSet<GitPathRootShaCached>, IOException> parents = r -> ImmutableSet
					.copyOf(graph.predecessors(r));
			final TPredicate<ImmutableSet<GitPathRootShaCached>, IOException> aAndC = s -> s.size() == 2
					&& (isA.test(s.asList().get(0)) && isC.test(s.asList().get(1)));
			final TPredicate<ImmutableSet<GitPathRootShaCached>, IOException> cAndA = s -> s.size() == 2
					&& (isC.test(s.asList().get(0)) && isA.test(s.asList().get(1)));
			final TPredicate<GitPathRootShaCached, IOException> twoRightParents = compose(parents, aAndC.or(cAndA));
			final TPredicate<GitPathRootShaCached, IOException> dBranch = compose(p -> GradeUtils.getRefsTo(refs, p),
					anyMatch(isRefBranch("br3")));
			...
			final CriterionGraderWeight<GitPathRootShaCached> graderBr3 = CriterionGraderWeight
					.given(Criterion.given("Branch br3"), dBranch, 2d);
			final CriterionGraderWeight<GitPathRootShaCached> graderContentSomeFile = CriterionGraderWeight.given(
			...
			final CriterionGraderWeight<GitPathRootShaCached> graderContentFirstApprox = CriterionGraderWeight
					.given(Criterion.given("approx"), compose(resolve("first.txt"), matchesApprox.and(merged)), 1d);
			final CriterionGraderWeight<GitPathRootShaCached> graderContentFirstExact = CriterionGraderWeight
					.given(Criterion.given("exact"),
							...
							1d);
			final CriterionGraderWeight<GitPathRootShaCached> graderContentFirst = CriterionGraderWeight.given(
					Criterion.given("content"),
					WeightingGrader
			...
			final TFunction<Optional<GitPathRootShaCached>, IGrade, IOException> grader = WeightingGrader
	...
	private TPredicate<Set<GitPathRootRef>, IOException> anyMatch(TPredicate<GitPathRootRef, IOException> predicate) {
	...
	private Optional<GitPathRootShaCached> getSingleParent(GitHistorySimple history, GitPathRootShaCached r) {
		final Set<GitPathRootShaCached> parents = history.graph().predecessors(r).stream()
