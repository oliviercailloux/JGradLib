...
import io.github.oliviercailloux.persons_manager.Person;
import io.github.oliviercailloux.persons_manager.PersonsManager;
import io.github.oliviercailloux.persons_manager.RedundancyCounter;
...
public class PersonsManagerGrader {
	...
	public static final ZonedDateTime DEADLINE = ZonedDateTime.parse("2021-03-31T23:59:59+01:00[Europe/Paris]");
	...
	public static void main(String[] args) throws Exception {
		...
		final GitGeneralGrader grader = GitGeneralGrader
				...
				.setExcludeCommitsByAuthors(ImmutableSet.of("Olivier Cailloux", "github-classroom[bot]"));
		grader.grade();
	...
	public IGrade grade() {
		...
		try (StdOutErrLogger redirecter = StdOutErrLogger.redirect()) {
			...
			gradeBuilder.add(CriterionGradeWeight.from(Criterion.given("Contains stream"), containsStream(), 2.5d));
			...
			gradeBuilder.add(CriterionGradeWeight.from(Criterion.given("To string"), toStringGrade(), 2d));
		}
	...
	private TryCatchAll<PersonsManager> getManager() {
		final TryCatchAll<PersonsManager> tryTarget = TryCatchAll
				.get(() -> instanciator.getInstanceOrThrow(PersonsManager.class, "empty", ImmutableList.of()))
				.or(() -> instanciator.getInstanceOrThrow(PersonsManager.class), (e1, e2) -> e1);
		return tryTarget.andApply(target -> SimpleTimeLimiter.create(executors).newProxy(target, PersonsManager.class,
				Duration.ofSeconds(5)));
	}

	private TryCatchAll<PersonsManager> getManagerGiven(Iterable<Person> arg) {
		return TryCatchAll
				.get(() -> instanciator.getInstanceOrThrow(PersonsManager.class, "given", ImmutableList.of(arg)));
	}
	...
	private WeightingGrade sizeAndStaticFactories() {
		final ImmutableSet.Builder<CriterionGradeWeight> gradeBuilder = ImmutableSet.builder();
		{
			final TryCatchAll<PersonsManager> manager = TryCatchAll
					.get(() -> instanciator.getInstanceOrThrow(PersonsManager.class, "empty", ImmutableList.of()));
		...
		{
			final TryCatchAll<Integer> sizeList = getManagerGiven(ImmutableList.of()).andApply(PersonsManager::size);
			final TryCatchAll<Integer> sizeSet = getManagerGiven(ImmutableMultiset.of()).andApply(PersonsManager::size);
...

		{
			...
			final TryCatchAll<Integer> size = getManagerGiven(ImmutableMultiset.copyOf(persons))
					.andApply(PersonsManager::size);
			final Mark mark = size.map(s -> Mark.binary(s == 100), PersonsManagerGrader::exc);
			gradeBuilder.add(CriterionGradeWeight.from(Criterion.given("Given 100 distinct persons"), mark, 0.5d));
		}
...

		{
			final ImmutableList<Person> persons = Streams
					.concat(IntStream.range(1000, 1100).boxed().map(i -> Person.given(455, "The person " + i)),
			...
			gradeBuilder.add(
					CriterionGradeWeight.from(Criterion.given("Set 200 persons with some duplicate ids"), mark, 1d));
		}

		{
			...
			gradeBuilder.add(CriterionGradeWeight.from(
					Criterion.given("Set 100 unique persons with some duplicates and duplicate ids"), mark, 0.5d));
		}
		return WeightingGrade.from(gradeBuilder.build());
	}
	...
	private WeightingGrade taskForce() {
		final ImmutableSet.Builder<CriterionGradeWeight> gradeBuilder = ImmutableSet.builder();
		{
			final TryCatchAll<Mark> managerSetMarked = getManager().andApply(m -> TryCatchAllVoid.run(m::setTaskForce)
					.map(() -> Mark.zero("Set succeeded but shouldn’t have."), t -> Mark
							.binary(t instanceof IllegalArgumentException, "", "Incorrect exception when set: " + t)));
...
//			final String comment = setFailed ? (rightExc ? "" : "Got exception: " + managerSet.getCause()) : "";
//			final Mark mark = Mark.binary(setFailed && rightExc, "", comment);
			gradeBuilder.add(CriterionGradeWeight
					.from(Criterion.given("Set empty task force, thrown IllegalArgumentException"), mark, 1d));
		}

		{
			...
			final Mark mark = managerSet.map(
					...
					t1 -> Mark.zero("Got init exception: " + t1));
			gradeBuilder.add(CriterionGradeWeight
					.from(Criterion.given("Set triple task force, thrown IllegalArgumentException"), mark, 1d));
		}

		{
			final TryCatchAll<PersonsManager> manager = getManager();
			final TryCatchAll<Integer> sizeStart = manager.andApply(PersonsManager::size);
			final TryCatchAll<PersonsManager> managerSet = manager.andConsume(m -> m.setTaskForce(personWithId(416)));
			...
			final Mark mark = sizeStart.and(size, (sS, s) -> Mark.binary(sS == 0 && s == 1))
					.orMapCause(PersonsManagerGrader::exc);
			gradeBuilder.add(CriterionGradeWeight.from(Criterion.given("Set singleton task force, size 1"), mark, 1d));
		}

		{
			final TryCatchAll<PersonsManager> manager = getManager();
			final TryCatchAll<Integer> sizeStart = manager.andApply(PersonsManager::size);
			final TryCatchAll<PersonsManager> managerSet = manager
					.andConsume(m -> m.setTaskForce(personWithId(416), personWithId(41)));
			final TryCatchAll<Integer> size = managerSet.andApply(PersonsManager::size);
			final Mark mark = sizeStart.and(size, (sS, s) -> Mark.binary(sS == 0 && s == 2))
					.orMapCause(PersonsManagerGrader::exc);
			gradeBuilder.add(CriterionGradeWeight.from(Criterion.given("Set dual task force, size 2"), mark, 1d));
		}
		return WeightingGrade.from(gradeBuilder.build());
	}
...
//			gradeBuilder
//					.add(CriterionGradeWeight.from(Criterion.given("Set to empty, does not contain empty name"), mark, 0.5d));
//		}

		{
			...
			final TryCatchAll<Boolean> containsRight100 = managerSet.andApply(m -> !m.contains("The person 100"));
			final TryCatchAll<Boolean> containsRight1000 = managerSet.andApply(m -> m.contains("The person 1000"));
			final TryCatchAll<Boolean> containsRight = containsRight100.and(containsRight1000,
					(r100, r1000) -> r100 && r1000);
			final Mark mark = containsRight.map(c -> Mark.binary(c, "", "Invalid contains"), PersonsManagerGrader::exc);
			gradeBuilder.add(CriterionGradeWeight.from(Criterion.given("Set 100 persons"), mark, 1d));
		}

		{
			...
			final Mark mark = containsRight.map(c -> Mark.binary(c, "", "Invalid contains"), PersonsManagerGrader::exc);
			gradeBuilder.add(CriterionGradeWeight
					.from(Criterion.given("Set 100 unique persons with some duplicates and duplicate ids"), mark, 1d));
		}
		return WeightingGrade.from(gradeBuilder.build());
	}
	...
	private WeightingGrade containsStream() {
		final ImmutableSet.Builder<CriterionGradeWeight> gradeBuilder = ImmutableSet.builder();
		{
			final ImmutableList<Person> persons = IntStream.range(1000, 1100).boxed()
					.map(i -> Person.given(i, "The person " + i)).collect(ImmutableList.toImmutableList());
			final TryCatchAll<PersonsManager> managerSet = getManagerWith(persons);
			final TryCatchAll<Boolean> containsRight1 = managerSet.andApply(m -> !m.contains(new MyInputStream("")));
			managerSet.andConsume(m -> m.setPersons(ImmutableList.of(Person.given(21, ""))));
			final TryCatchAll<Boolean> containsRight2 = managerSet.andApply(m -> m.contains(new MyInputStream("")));
			final TryCatchAll<Boolean> containsRight = containsRight1.and(containsRight2, (b1, b2) -> b1 && b2);
			final Mark mark = containsRight.map(c -> Mark.binary(c, "", "Invalid contains"), PersonsManagerGrader::exc);
			gradeBuilder.add(CriterionGradeWeight.from(Criterion.given("Contains stream empty name"), mark, 1d));
		}

		{
			...
			final ImmutableList<Person> persons2 = Streams
					.concat(IntStream.range(1000, 1100).boxed().map(i -> Person.given(455, "The person " + i)),
			...
			final Mark markContains = containsRight.map(c -> Mark.binary(c, "", "Invalid contains"),
					PersonsManagerGrader::exc);
			final Mark markAll = containsRight.map(c -> Mark.binary(c && !stream1.wasClosed() && !stream2.wasClosed()),
					PersonsManagerGrader::exc);
			gradeBuilder.add(CriterionGradeWeight.from(Criterion.given("Contains stream Asbjørn Nuñez"),
					WeightingGrade.from(ImmutableSet.of(
							...
							CriterionGradeWeight.from(Criterion.given("Contains and not closed"), markAll, 0.5d))),
					1d));
		}
		return WeightingGrade.from(gradeBuilder.build());
	}
	...
	private WeightingGrade map() {
		final ImmutableSet.Builder<CriterionGradeWeight> gradeBuilder = ImmutableSet.builder();
		{
			...
			final ImmutableList<Person> persons = Streams
					...
					.collect(ImmutableList.toImmutableList());
			final TryCatchAll<PersonsManager> managerSet = getManagerWith(persons);
			final TryCatchAll<Mark> managerSetMarked = managerSet.andApply(
					m -> TryCatchAllVoid.run(m::toMap).map(() -> Mark.zero("toMap succeeded but shouldn’t have."),
							t -> Mark.binary(
									(t instanceof IllegalArgumentException) || (t instanceof IllegalStateException), "",
									"Incorrect toMap exception: " + t)));
			final Mark mark = managerSetMarked.orMapCause(t1 -> Mark.zero("Got init exception: " + t1));
			gradeBuilder
					.add(CriterionGradeWeight.from(Criterion.given("Set a duplicate id, toMap throwing"), mark, 1d));
		}

		{
			...
			final ImmutableMap<Integer, Person> mapPersons = Maps.toMap(
					IntStream.range(1000, 1100).boxed().collect(ImmutableSet.toImmutableSet()),
					PersonsManagerGrader::personWithId);
			final TryCatchAll<Boolean> mapEquals = map.andApply(m -> m.equals(mapPersons));
			final TryCatchAll<Map<Integer, Person>> withPut = map.andConsume(m -> m.put(1, personWithId(1)));
			final Mark markContent = Mark
					.binary(mapSize.equals(TryCatchAll.success(100)) && mapEquals.equals(TryCatchAll.success(true)));
			final Mark markImmutable = Mark.binary(map.isSuccess() && withPut.isFailure());
			gradeBuilder.add(
					CriterionGradeWeight.from(Criterion.given("Set 100 unique persons without duplicate ids, toMap"),
	...
	private WeightingGrade iterator() {
		final ImmutableSet.Builder<CriterionGradeWeight> gradeBuilder = ImmutableSet.builder();
		{
			...
			gradeBuilder.add(CriterionGradeWeight.from(Criterion.given("Set 100 unique persons with no duplicate ids"),
					mark, 1d));
		}

		{
			...
			final TryCatchAll<Iterator<Person>> itL = it.andApply(i -> Iterators.limit(i, 1000));
			final TryCatchAll<ImmutableMultiset<Person>> result = itL.andApply(ImmutableMultiset::copyOf);
			final ImmutableMultiset<Person> expected = ImmutableMultiset.copyOf(ImmutableSet.copyOf(persons));
			final TryCatchAll<Boolean> resultRight = result.andApply(l -> l.equals(expected));
			final Mark mark = resultRight.map(b -> Mark.binary(b), PersonsManagerGrader::exc);
			gradeBuilder.add(CriterionGradeWeight
					.from(Criterion.given("Set 150 unique persons with some duplicates and duplicate ids"), mark, 1d));
		}
		return WeightingGrade.from(gradeBuilder.build());
	}
	...
	private WeightingGrade idIterator() {
		...
		{
			final ImmutableList<Person> persons = Streams
					.concat(IntStream.range(1000, 1020).boxed(), IntStream.range(1100, 1200).boxed())
					.map(PersonsManagerGrader::personWithId).collect(ImmutableList.toImmutableList());
			final TryCatchAll<PersonsManager> managerSet = getManagerWith(persons);
			...
			final TryCatchAll<ImmutableMultiset<Integer>> result = itL.andApply(ImmutableMultiset::copyOf);
			final ImmutableMultiset<Integer> expected = persons.stream().map(Person::getId)
	...
	private WeightingGrade counter() {
		final ImmutableSet.Builder<CriterionGradeWeight> gradeBuilder = ImmutableSet.builder();
		final TryCatchAll<PersonsManager> manager = getManagerWith(ImmutableList.of());
		final TryCatchAll<RedundancyCounter> counter = manager.andApply(PersonsManager::getRedundancyCounter);
		{
			final TryCatchAll<Integer> redundant = counter.andApply(RedundancyCounter::getRedundancyCount);
			final TryCatchAll<Integer> unique = counter.andApply(RedundancyCounter::getUniqueCount);
			final Mark mark = Mark
					.binary(redundant.equals(TryCatchAll.success(0)) && unique.equals(TryCatchAll.success(0)));
			gradeBuilder.add(CriterionGradeWeight.from(Criterion.given("Start with 0 redundancy count"), mark, 0.5d));
		}
		manager.andConsume(m -> m.setPersons(ImmutableList.of(personWithId(2))));
		{
			...
			final TryCatchAll<Integer> unique = counter.andApply(RedundancyCounter::getUniqueCount);
			final Mark mark = Mark
					.binary(redundant.equals(TryCatchAll.success(0)) && unique.equals(TryCatchAll.success(1)));
			gradeBuilder.add(CriterionGradeWeight.from(Criterion.given("Switch to 1 unique count"), mark, 1d));
		}
		final ImmutableList<Person> persons = Streams
		...
		manager.andConsume(m -> m.setPersons(persons));
		{
			...
			final TryCatchAll<Integer> unique = counter.andApply(RedundancyCounter::getUniqueCount);
			final Mark mark = Mark
					.binary(redundant.equals(TryCatchAll.success(10)) && unique.equals(TryCatchAll.success(120)));
			gradeBuilder.add(CriterionGradeWeight.from(Criterion.given("Switch to “many” count"), mark, 1d));
		}
		return WeightingGrade.from(gradeBuilder.build());
	}
	...
	private WeightingGrade toStringGrade() {
		...
		final TryCatchAll<PersonsManager> managerSet = getManagerWith(
				ImmutableList.of(personWithId(21), personWithId(24)));
		final TryCatchAll<String> str = managerSet.andApply(PersonsManager::toString);
		final Mark mark = str.map(s -> Mark.binary(s.equals("PersonsManager with 2 entries"), "", "Got: " + s),
				PersonsManagerGrader::exc);
		gradeBuilder.add(CriterionGradeWeight.from(Criterion.given("Set to two unique persons"), mark, 1d));
