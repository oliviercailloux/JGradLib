...
public class Patcher {
    ...
    public static void main(String[] args) throws Exception {
        ...
        final Type typePatch = new LinkedHashMap<String, LinkedHashSet<Patch>>() {}.getClass()
        ...
        final Map<String, IGrade> grades =
                JsonbUtils.fromJson(Files.readString(Path.of("grades " + prefix + ".json")), type,
                        JsonGrade.instance());

        final ImmutableMap<String,
                Set<Patch>> completedPatches = grades.entrySet().stream()
                        .collect(ImmutableMap.toImmutableMap(Map.Entry::getKey,
                                e -> patches.containsKey(e.getKey()) ? patches.get(e.getKey())
                                        : ImmutableSet.of()));
