...
public class SendEmails {
	...
	public static void main(String[] args) throws Exception {
		...
		final ImmutableMap<EmailAddressAndPersonal,
				MarksTree> marksByEmail = exam.getUsernames().stream().collect(ImmutableMap.toImmutableMap(
		...
		final ImmutableList<Double> points = gradesByEmail.values().stream().map(Grade::mark)
		...
		try (GradesInEmails gradesInEmails = GradesInEmails.newInstance()) {
			...
			final ImmutableMap<EmailAddress, Grade> lastGrades =
			...
			final Map<EmailAddressAndPersonal,
					Grade> gradesDiffering = gradesByEmail.entrySet().stream()
			...
			final ImmutableSet<Email> emails = gradesDiffering
