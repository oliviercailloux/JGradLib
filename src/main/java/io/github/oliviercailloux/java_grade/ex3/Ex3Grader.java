...
//				.collect(Collectors.summingDouble(Criterion::getMaxPoints));
		...
		gradesBuilder.add(Grade.binary(AT_ROOT, pomSupplier.isMavenProjectAtRoot()));

		final PomContexter pomContexter = new PomContexter(pomContent);
		pomContexter.init();

		gradesBuilder.add(Grade.binary(GROUP_ID, pomContexter.isGroupIdValid()));
		gradesBuilder.add(Grade.binary(JUNIT5_DEP,
				MarkingPredicates.containsOnce(Pattern.compile("<dependencies>" + Utils.ANY_REG_EXP + "<dependency>"
						...
						.test(pomContent)));
		gradesBuilder.add(Grade.binary(UTF,
		...
		gradesBuilder.add(Grade.binary(NO_MISLEADING_URL,
				Predicates.contains(Pattern.compile("<url>.*\\.apache\\.org.*</url>")).negate().test(pomContent)));
		gradesBuilder.add(Grade.binary(WAR,
		...
		gradesBuilder.add(Grade.binary(NO_JSP, JavaEEMarkers.getNoJsp(filesReader)));
		gradesBuilder.add(Grade.binary(NO_WEB_XML, JavaEEMarkers.getNoWebXml(filesReader)));
		gradesBuilder.add(Grade.binary(DO_GET, servletSourcer.existsAndAllMatch(MarkingPredicates
				.containsOnce(Pattern.compile("void\\s*doGet\\s*\\(\\s*(final)?\\s*HttpServletRequest .*\\)")))));
		gradesBuilder.add(Grade.binary(NO_DO_POST,
		...
		gradesBuilder.add(Grade.binary(NOT_POLLUTED,
		...
		gradesBuilder.add(Grade.binary(GET_HELLO,
		...
		gradesBuilder
				.add(Grade.binary(TEST_LOCATION, testSourcer.getContents().keySet().stream().allMatch(MarkingPredicates
...
//		gradesBuilder.add(Marks.mavenTest(TEST_GREEN, fullContext, testSourcer, pomSupplier));
		gradesBuilder.add(Grade.binary(ASSERT_EQUALS, testSourcer.anyMatch(Predicates
		...
		gradesBuilder.add(Grade.binary(TRAVIS_CONF, !travisContent.isEmpty()));
		...
		gradesBuilder.add(Grade.binary(TRAVIS_BADGE, Pattern.compile(
