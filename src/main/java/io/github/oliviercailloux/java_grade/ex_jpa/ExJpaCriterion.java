...
public enum ExJpaCriterion implements CriterionAndPoints, Criterion {
	...
	SOURCE("Maven compiler source property", 0d, -0.5d / 20d),
	NO_MISLEADING_URL("No misleading url pointer", 0d, -0.5d / 20d), WAR("war packaging", 0d, -0.5d / 20d),
	PREFIX("All classes are prefixed by the group id", 0d, -0.5d / 20d),
	COMPILE("mvn compile (without tests)", 1.5d, 0d), NO_JSP("No JSP files", 0d, -0.5d / 20d),
	NO_WEB_XML("No web.xml file", 0d, -0.5d / 20d),
	GET("Only method @GET in GetCommentsServlet, and class named correctly", 0.5d),
	POST("Only method @POST in AddCommentServlet", 1d),
	NOT_POLLUTED("No noisy auto-generated code, comments or tests", 0d, -1d / 20d),
	EXC("Exceptions are not hidden", 0d, -2d / 20d),
	...
	ONLY_ORIG("No derivative files in repo", 0d, -0.5d / 20d),
	...
	PERSISTENCE_CONTENTS(
			...
			1d),
	USING_EM("Injection of an EntityManager", 2d),
	...
	TEST_EXISTS("Some unit tests provided (and non empty) and are in the right place", 0.5d),
	GENERAL_TEST(
