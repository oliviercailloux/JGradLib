...
public class RepositoryWithIssuesWithHistory {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(RepositoryWithIssuesWithHistory.class);
  ...
  private RepositoryWithIssuesWithHistory(JsonObject json) {
    ...
    issues = GitHubJsonParser.getContent(issuesConnection, true).map(IssueBare::from)
        .map(this::getIssue).collect(ImmutableList.toImmutableList());
    {
      final Builder<String, IssueWithHistory> builderByOriginalName =
          ImmutableSetMultimap.<String, IssueWithHistory>builder()
              .orderValuesBy(Comparator.<IssueWithHistory>naturalOrder());
    ...
    {
      final Builder<String, IssueWithHistory> builderByName =
          ImmutableSetMultimap.<String, IssueWithHistory>builder()
              .orderValuesBy(Comparator.<IssueWithHistory>naturalOrder());
      for (IssueWithHistory issue : issues) {
    ...
    }
    {
      files = json.isNull("masterObject") ? ImmutableList.of()
          : json.getJsonObject("masterObject").getJsonArray("entries").stream()
              .map(JsonValue::asJsonObject).filter((e) -> e.getString("type").equals("blob"))
              .map((e) -> e.getString("name")).collect(ImmutableList.toImmutableList());
    }
  ...
  /**
   * Returns all the issues that have that name as original name or a name corresponding to the same
   * functionality, for example, "function-2" corresponds to the name "function". The returned set
   * is ordered by alphabetic order of name. No two issues in the set have the same name. In case of
   * homonyms, the issue kept for a given name is the one that has the earliest date of “first
   * done”.
   *
   * @return may be empty.
   */
  public ImmutableSortedSet<IssueWithHistory> getIssuesCorrespondingTo(String name) {
    checkState(issuesComplete);
    final Collector<IssueWithHistory, ?, ImmutableSortedSet<IssueWithHistory>> collector =
        ImmutableSortedSet.toImmutableSortedSet(Comparator.naturalOrder());
    final Map<String, IssueWithHistory> collected =
        issues.stream().filter((i) -> i.getOriginalName().matches(name + "(-[2-9]+\\d*)?"))
            .collect(Collectors.groupingBy(IssueWithHistory::getOriginalName,
                Collectors.collectingAndThen(collector, (l) -> l.iterator().next())));
    final ImmutableSortedSet<IssueWithHistory> issuesM =
        collected.values().stream().collect(ImmutableSortedSet
            .toImmutableSortedSet(Comparator.comparing(IssueWithHistory::getOriginalName)));
  ...
  /**
   * Returns all the issues that have ever had that name, ordered by their date of “first done”
   * (earliest first), with any issues that do not have “first done” coming last.
   *
   * @return may be empty.
   */
  ...
  /**
   * Returns all the issues that have that name as original name, ordered by their date of “first
   * done” (earliest first), with any issues that do not have “first done” coming last.
   *
   * @return may be empty.
   */
  public ImmutableSortedSet<IssueWithHistory> getIssuesOriginallyNamed(String name) {
     ...
     *
     * TODO bug apparently if an issue is called WSCallRank and another is called WSCall (see
     * XM-GUI).
     */
    requireNonNull(name);
  ...
  private IssueWithHistory getIssue(IssueBare issueBare) {
    ...
    final String name;
    {
      final Optional<RenamedTitleEvent> firstRename =
          events.stream().filter((e) -> e instanceof RenamedTitleEvent)
              .map((e) -> (RenamedTitleEvent) e).findFirst();
