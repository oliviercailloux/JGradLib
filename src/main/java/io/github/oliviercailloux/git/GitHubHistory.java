...
public class GitHubHistory {
   ...
   */
  private static ImmutableMap<ObjectId, ObjectId> getLoweringPatchForNonIncreasing(
      Graph<ObjectId> graph, Map<ObjectId, Instant> initial, Comparator<Instant> comparator) {
  ...
  private void checkAndCompletePushDates() {
    ...
    final Set<ObjectId> nodes = graph.nodes();
    final ImmutableSet<ObjectId> unobservedPushedDates =
    ...
    final ImmutableMap<ObjectId, ObjectId> patch =
        getLoweringPatchForNonIncreasing(graph, initial, comparator);

    final ImmutableMap<ObjectId, Instant> modifiedPushedDates =
    ...
    final ImmutableMap<ObjectId, ObjectId> unknownsPatch = getLoweringPatchForNonIncreasing(
